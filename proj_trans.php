...
if ($result->num_rows > 0) {
    ...
    echo "<table border=1>";
    echo "<tr><td>ID</td><td>Transaction Type</td><td>Type ID</td>
        <td>Transaction Costs($)</td><td>Balance Due($)</td><td>Transaction Date</td></tr>";
    // output data of each row
    while ($row = $result->fetch_assoc()) {
        echo "<tr><td>" . $row["transID"] . "</td><td>" . $row["transType"] . "</td><td>" .$row["typeID"] . "</td><td>" .
